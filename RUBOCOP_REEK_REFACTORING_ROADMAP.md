...
- Split `OPFProcessor` into focused helpers: `OPFEntryReader` for entry IO/path resolution, `OPFNavigationExtractor` for nav/NCX traversal, and `OPFMetadataExtractor` for metadata parsing; simplified `process_spine` to yield a `SpineItem` value object and updated `EpubImporter` accordingly.
- Introduced `XmlTextNormalizer` to centralize XML encoding normalization/sanitization and replaced duplicated logic in `EpubImporter`.
- Refactored OPF navigation helpers (source locator, traversal, selector, walker, context) and added `OPFNavigationDocumentScanner` + `OPFNavigationListItem`; updated `OPFNavigationDocumentIndex` to use the scanner and cleaned nav helper Reek/RuboCop warnings (`bundle exec reek` + `bundle exec rubocop` on nav helper files).
- Refactored `BookCachePipeline` cache loading flow: added `PointerRebuilder`, `FingerprintFilter`, `CacheStatus`, and `PayloadContext` helpers, simplified fast-load pointer lookup, and reduced CacheSession data clumps/conditionals; `bundle exec reek lib/ebook_reader/infrastructure/book_cache_pipeline.rb` now clean.
- Renamed rescue variables in `BookCachePipeline` to remove remaining `UncommunicativeVariableName` warnings; `bundle exec reek lib/ebook_reader/infrastructure/book_cache_pipeline.rb` remains clean.
