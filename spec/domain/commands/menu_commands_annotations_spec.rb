...
RSpec.describe EbookReader::Domain::Commands::MenuCommand do
  ...
  class FakeAnnotationsScreen
    ...
    def navigate(dir); @navigations << dir; end
    def current_annotation = @current_annotation
    def current_book_path = @current_book_path
  end
  ...
  class Ctx
    ...
    def open_selected_annotation; @calls << :open_selected_annotation; end
    def open_selected_annotation_for_edit; @calls << :open_selected_annotation_for_edit; end
    def delete_selected_annotation; @calls << :delete_selected_annotation; end
    def switch_to_mode(mode); @calls << [:switch_to_mode, mode]; end
  end
  ...
  it 'navigates annotation list up/down' do
    ...
    expect(ctx.main_menu_component.annotations_screen.navigations).to eq([:up, :down])
  end
  ...
  it 'selects annotation and switches to detail' do
    ...
    expect(ctx.calls).to include([:switch_to_mode, :annotation_detail])
  end
  ...
  it 'handles annotation detail actions' do
    ...
    expect(ctx.calls).to include([:switch_to_mode, :annotations])
  end
end
