...
RSpec.describe EbookReader::Domain::Services::Internal::DynamicPageMapBuilder do
  ...
  it 'keeps image blocks together when paginating' do
    ...
    expect(formatter).to receive(:wrap_all).with(doc, 0, 80, config: nil, lines_per_page: 6).and_return(wrapped)
    ...
    expect(pages[0][:lines].length).to eq(4)
    expect(pages[0][:lines].any? { |line| line.respond_to?(:metadata) && line.metadata[:block_type] == :image }).to eq(false)

    expect(pages[1][:start_line]).to eq(4)
    expect(pages[1][:end_line]).to eq(7)
    expect(pages[1][:lines].length).to eq(4)
    expect(pages[1][:lines].count { |line| line.respond_to?(:metadata) && line.metadata[:block_type] == :image }).to eq(3)
