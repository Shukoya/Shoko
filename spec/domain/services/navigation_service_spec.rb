...
RSpec.describe EbookReader::Domain::Services::NavigationService do
  ...
  describe '#next_page' do
    context 'with single view mode' do
      ...
      it 'advances to next page within chapter' do
        expect(state_store).to receive(:set).with(%i[reader current_page], 6)
    ...
    context 'with split view mode' do
      ...
      it 'advances by 2 pages for split view' do
        expect(state_store).to receive(:set).with(%i[reader current_page], 7)
  ...
  describe '#prev_page' do
    context 'with single view mode' do
      ...
      it 'goes back to previous page' do
        expect(state_store).to receive(:set).with(%i[reader current_page], 4)
  ...
  describe '#jump_to_chapter' do
    ...
    it 'validates chapter index' do
      ...
      end.to raise_error(ArgumentError, 'Chapter index must be non-negative')
    end
    ...
    context 'when chapter index exceeds total chapters' do
      ...
      it 'raises error for invalid chapter index' do
        ...
        end.to raise_error(ArgumentError, /Chapter index 5 exceeds total chapters 3/)
      end
    end
  end
  ...
  describe '#go_to_end' do
    ...
    it 'goes to end of book' do
      expect(state_store).to receive(:update).with({
                                                     %i[reader current_chapter] => 4,
                                                     %i[reader current_page] => 20,
                                                   })
  ...
  describe '#scroll' do
    ...
    it 'scrolls up by specified lines' do
      expect(state_store).to receive(:set).with(%i[reader current_page], 3)
    ...
    it 'prevents scrolling above beginning' do
      ...
      expect(state_store).to receive(:set).with(%i[reader current_page], 0)

      service.scroll(:up, 5)
    end

    it 'prevents scrolling beyond end' do
      expect(state_store).to receive(:set).with(%i[reader current_page], 10)
    ...
    it 'validates scroll direction' do
      ...
      end.to raise_error(ArgumentError, 'Invalid scroll direction: invalid')
    end
  end
end
