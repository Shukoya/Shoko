...
RSpec.describe EbookReader::Infrastructure::StateStore do
  ...
  describe '#initialize' do
    it 'creates initial state' do
      expect(state_store.get([:reader, :current_chapter])).to eq(0)
      expect(state_store.get([:reader, :view_mode])).to eq(:split)
      expect(state_store.get([:config, :theme])).to eq(:dark)
    end
  end
  ...
  describe '#get' do
    it 'retrieves values by path' do
      expect(state_store.get([:reader, :current_chapter])).to eq(0)
      expect(state_store.get([:menu, :selected_index])).to eq(0)
    end
    
    it 'returns nil for non-existent paths' do
      expect(state_store.get([:non, :existent, :path])).to be_nil
    end
  ...
  describe '#set' do
    it 'updates single path' do
      state_store.set([:reader, :current_chapter], 5)
      
      expect(state_store.get([:reader, :current_chapter])).to eq(5)
    end
    
    it 'emits change event' do
      expect(event_bus).to receive(:emit_event).with(:state_changed, {
        path: [:reader, :current_chapter],
        old_value: 0,
        new_value: 5,
        full_state: anything
      })
    ...
    it 'does not emit event for same value' do
      ...
      state_store.set([:reader, :current_chapter], 0) # Same as initial value
    end
  end
  ...
  describe '#update' do
    it 'updates multiple paths' do
      ...
      expect(state_store.get([:reader, :current_chapter])).to eq(3)
      expect(state_store.get([:reader, :current_page])).to eq(10)
      expect(state_store.get([:config, :view_mode])).to eq(:single)
    end
    ...
    it 'does not emit events for unchanged values' do
      ...
      updates = {
        [:reader, :current_chapter] => 0, # Same as current
        [:reader, :current_page] => 10    # Different
      }
  ...
  describe '#current_state' do
    ...
    it 'reflects updates' do
      state_store.set([:reader, :current_chapter], 7)
  ...
  describe '#reset!' do
    it 'resets to initial state' do
      state_store.set([:reader, :current_chapter], 10)
  ...
  describe 'validation' do
    it 'validates chapter index is non-negative' do
      expect {
        state_store.set([:reader, :current_chapter], -1)
      }.to raise_error(ArgumentError, 'current_chapter must be non-negative')
    end
    
    it 'validates view mode is valid' do
      expect {
        state_store.set([:reader, :view_mode], :invalid)
      }.to raise_error(ArgumentError, 'invalid view_mode')
    end
    
    it 'validates terminal dimensions are positive' do
      expect {
        state_store.set([:ui, :terminal_width], 0)
      }.to raise_error(ArgumentError, 'terminal dimensions must be positive')
    end
  end
  ...
  describe 'thread safety' do
    it 'handles concurrent updates safely' do
      threads = 10.times.map do |i|
        Thread.new do
          state_store.set([:reader, :current_chapter], i)
        end
      end
      ...
      chapter = state_store.get([:reader, :current_chapter])
