    ...
    class CommandFactory
      class << self
        ...
        def create(action, *)
          ...
          # Mode switching
          when :show_help
            ModeCommand.new(:help, *)
          when :open_toc
            ModeCommand.new(:toc, *)
          when :open_bookmarks
            ModeCommand.new(:bookmarks, *)
          when :open_annotations
            ModeCommand.new(:annotations, *)
        ...
        def create_browse_bindings
          ...
          Input::KeyDefinitions::NAVIGATION[:up].each do |k|
            bindings[k] = lambda { |ctx, key|
              ctx.handle_browse_navigation(key)
              :handled
            }
          end
          Input::KeyDefinitions::NAVIGATION[:down].each do |k|
        ...
        def create_recent_bindings
          ...
          Input::KeyDefinitions::NAVIGATION[:up].each do |k|
            bindings[k] = lambda { |ctx, key|
              ctx.input_handler.handle_recent_input(key)
              :handled
            }
          end
          Input::KeyDefinitions::NAVIGATION[:down].each do |k|
