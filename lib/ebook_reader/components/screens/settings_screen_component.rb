    ...
    module Screens
      ...
      class SettingsScreenComponent < BaseComponent
        ...
        def do_render(surface, bounds)
          ...
          render_setting_option(surface, bounds, 6, "4", "Page Numbering Mode", format_page_numbering_mode)
        ...
        def format_view_mode
          case @state.get([:config, :view_mode])
        ...
        def format_page_numbers
          @state.get([:config, :show_page_numbers]) ? "Enabled" : "Disabled"
        end

        def format_page_numbering_mode
          case @state.get([:config, :page_numbering_mode])
        ...
        def format_highlighting
          quotes = @state.get([:config, :highlight_quotes]) ? "Quotes" : ""
          keywords = @state.get([:config, :highlight_keywords]) ? "Keywords" : ""
