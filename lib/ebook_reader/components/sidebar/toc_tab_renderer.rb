      ...
      class TocTabRenderer < BaseComponent
        ...
        def render_empty_message(surface, bounds)
          messages = [
            'No chapters found',
            '',
            "#{COLOR_TEXT_DIM}Content may still be loading#{Terminal::ANSI::RESET}",
          ]
        ...
        def render_filter_input(surface, bounds, state)
          ...
          input_text = "#{COLOR_TEXT_PRIMARY}#{filter_text}#{Terminal::ANSI::RESET}"
          ...
          help_text = "#{COLOR_TEXT_DIM}ESC to cancel#{Terminal::ANSI::RESET}"
