    ...
    module Reading
      ...
      module InlineSegmentHighlighter
        ...
        def build_ranges(text, highlight_quotes, highlight_keywords)
          quote_ranges = highlight_quotes ? match_ranges(text, EbookReader::Constants::QUOTE_PATTERNS) : []
          keyword_ranges = highlight_keywords ? match_ranges(text, EbookReader::Constants::HIGHLIGHT_PATTERNS) : []
        ...
        class RangeApplicator
          ...
          def segment_pieces(boundaries, base_styles)
            boundaries.each_cons(2).filter_map do |start_index, end_index|
              ...
              EbookReader::Domain::Models::TextSegment.new(text: piece, styles: styles)
            end
          end
