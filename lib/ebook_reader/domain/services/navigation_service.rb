      ...
      class NavigationService < BaseService
        ...
        def go_to_end
          ...
          @state_store.update({
                                ...
                                %i[reader current_page] => calculate_last_page(last_chapter),
                              })
        end
        ...
        def scroll(direction, lines = 1)
          ...
          @state_store.set(%i[reader current_page], new_page) if new_page != current_page
        end
        ...
        def next_page_split(state)
          ...
          if current_page + 2 <= max_page
            @state_store.set(%i[reader current_page], current_page + 2)
          elsif can_advance_chapter?(state)
            jump_to_chapter(current_chapter + 1)
          end
        end
        ...
        def prev_page_split(state)
          ...
          if current_page >= 2
            ...
            @state_store.update({
                                  %i[reader current_chapter] => prev_chapter_index,
                                  %i[reader current_page] => aligned_page,
                                })
          end
        end
